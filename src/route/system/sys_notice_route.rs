...
pub fn build_sys_notice_route() -> Router<Arc<AppState>> {
    Router::new()
        .route(
            "/system/notice/addNotice",
            post(sys_notice_handler::add_sys_notice),
        )
        .route(
            "/system/notice/deleteNotice",
            post(sys_notice_handler::delete_sys_notice),
        )
        .route(
            "/system/notice/updateNotice",
            post(sys_notice_handler::update_sys_notice),
        )
        .route(
            "/system/notice/updateNoticeStatus",
            post(sys_notice_handler::update_sys_notice_status),
        )
        .route(
            "/system/notice/queryNoticeDetail",
            post(sys_notice_handler::query_sys_notice_detail),
        )
        .route(
            "/system/notice/queryNoticeList",
